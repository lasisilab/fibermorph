...
def teardown_module(function):
    teardown_files = ["empty_file1.txt", "test1/empty_file1.txt"]
...
def test_length_measurement():
    ref_df = pd.read_csv(
        pathlib.Path(
            "fibermorph/test_data/SimArcData/Oct06_1855_32_017780_arc_data.csv"
        ),
        header=0,
        index_col=0,
    )
    ...
    img, name = fibermorph.imread(
        pathlib.Path(
            "fibermorph/test_data/SimArcData/Oct06_1855_32_017780_arc_data.tiff"
        )
    )
    ...
    label_image, num_elements = skimage.measure.label(
        prun_im.astype(int), connectivity=2, return_num=True
    )
...
def section_props(props, im_name, resolution, minpixel, maxpixel, im_center):
    ...
    props_df = [
        ...
        for region in props
        if minpixel <= region.area <= maxpixel
    ]
...
def test_segment_section():
    ...
    input_directory = pathlib.Path(
        "/Users/tinalasisi/Box/01_TPL5158/Box_Dissertation/HairPhenotyping_Methods/data/fibermorph_input/admixed_real_hair/section/AfrEu_SectionImages_RawJPG/AfrEu_SectionImages_GrayTIFF/tiff"
    )
